...
impl<Letter: AutLetter> AutAccessible for AutGNFA<Letter> {
    ...
    fn make_accessible(mut self) -> Self {
        let new_start_state = 0;
        let new_accept_state = 1;
    ...
    fn make_coaccessible(self) -> Self {
        ...
        let mut new_transitions : HashMap<(usize,usize), TermBRE<Letter>> = hashmap!{};
        let mut states_map = hashmap!{self.start_state => 0, self.accept_state => 1};
        let mut stack = vec![self.accept_state];
        while !stack.is_empty() {
            let targ_in_old = stack.pop().unwrap();
            let targ_in_new : usize = *states_map.get(&targ_in_old).unwrap();
            for orig_in_old in 0..self.states_num {
                if let Some(term) = self.transitions.get(&(orig_in_old,targ_in_old)) {
                    if !term.is_empty() {
                        let orig_in_new = match states_map.get(&orig_in_old) {
                            None => {
                                let orig_in_new = new_states_num;
                                new_states_num += 1;
                                states_map.insert(orig_in_old,orig_in_new);
                                stack.push(orig_in_old);
                                orig_in_new
                            },
                            Some(orig_in_new) => {
                                *orig_in_new
    ...
    fn trim(self) -> Self {
        self.make_coaccessible().make_accessible()
    }
